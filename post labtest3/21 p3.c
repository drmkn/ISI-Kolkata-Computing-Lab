...
int main(){
    //enough to check that reverse of post order 
    //sequence of mirror tree is same as that of pre of BST
    int m,n,flag;scanf("%d",&m);scanf("%d",&n);
    if(m != n){
        printf("NOT MIRROR TWIN");
        return 0;
    }
    int O_pre[m],M_in[n],M_post[n];
    for(int i = 0;i<m;i++){
        scanf("%d",&O_pre[i]);
    }
    for(int i = 0;i<2*n;i++){
        if(i < n) scanf("%d",&M_in[i]);
        else scanf("%d",&M_post[i%n]);
    }
    for(int i = 0;i<m;i++){
        if(O_pre[i] != M_post[m-i-1]){printf("NOT MIRROR TWIN");return 0;}
        else continue;
    }
    printf("MIRROR TWIN");   
    return 0;
