...
int dfs_search_word_count(int index){
    int count = 0;
    //for(int i = 0;i<26;i++) sum = trie[index][i]; 
...
int main(int ac, char **av){
    int w; char Buffer[256];
    FILE *prefixes,*strings;
    w = init_trie();
    prefixes = fopen(av[1],"r");int n;
    strings = fopen(av[2],"r");int m;
    fscanf(prefixes,"%d",&n);fscanf(strings,"%d",&m);
    for(int i = 0;i<m;i++){
        fscanf(strings,"%s",Buffer);
