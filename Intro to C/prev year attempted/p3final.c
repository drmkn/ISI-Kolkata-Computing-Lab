...
int string_compare(char *a,char *b){ //returns smaller string - 1 for first string,-1 for second string and 0 for equal string
     int l_a = strlen(a);int l_b = strlen(b);int result;int i;int flag=0;
     if(l_a < l_b){result = -1;}
     else if(l_a > l_b){result = 1;}
     else{
     for(i = 0;i<l_a;i++ ){
         if(*(a+i) - '0' < *(b+i) - '0'){result = -1;flag++;}
         else if(*(a+i) - '0' > *(b+i) - '0'){result = 1;flag++;}
         else{continue;}
     }
     if(flag == 0){
          result = 0;
...
int main(int ac, char **av){
    ...
    for(i=0;i<N;i++){ //sorting the strings
     for(j=i;j<N;j++){
          if(string_compare(*(pp+i),*(pp+j)) == 0 || string_compare(*(pp+i),*(pp+j)) == -1){continue;}
          else{temp = *(pp+i);
               *(pp+i) = *(pp+j);
               *(pp+j) = temp;}
     }
    ...
    //printf("%d",string_compare("000","101"));
    int count = 0;
    i = 0;
    j = 0;
